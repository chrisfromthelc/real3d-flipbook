...
FLIPBOOK.BookScroll = class extends FLIPBOOK.Book {
  ...
  fitToHeight() {
    this.iscroll.zoom(
      (this.zoom * this.main.wrapperH) / this.pageHeight,
      0,
      0,
      0,
    );
  ...
  fitToWidth() {
    this.iscroll.zoom(
      (this.zoom * this.main.wrapperW) / this.pageWidth,
      0,
      0,
      0,
    );
  ...
  fit() {
    if (this.options.fitToWidth) {
      ...
      this.main.wrapperW / this.main.wrapperH <
...
FLIPBOOK.PageScroll = class {
  constructor(book, bookWrapper, main, options, index, texture, html) {
    ...
    this.html.style.width =
      (1000 * this.options.pageWidth) / this.options.pageHeight + "px";
    this.html.style.transform =
      "scale(" + this.options.pageHeight / 1000 + ") translateZ(0)";
  ...
  load(callback, thumb) {
    ...
    var index = this.options.rightToLeft
    ...
    this.options.main.loadPage(index, size, function (page) {
      ...
      if (page && page.image) {
        ...
        if (
          self.index % 2 == 0 &&
          (self.options.pages[index].side == "left" ||
            self.options.pages[index].side == "right")
        ) {
          if (!img.clone) {
            img.clone = new Image();
        ...
        if (self.options.doublePage) {
          if (
            self.index == 0 ||
            (self.index == self.options.pages.length - 1 &&
              self.options.backCover)
          ) {
            img.style.width = "100%";
          } else {
            img.style.width = "200%";
