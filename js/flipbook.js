...
FLIPBOOK.escapeHtml = function (str) {
...
FLIPBOOK.Main = class {
    static defaultOptions = {
        ...
        strings: {
            ...
            toggleSinglePage: 'Toggle single page',
            close: 'Close',
    ...
    constructor(options, elem) {
        // Fix 1: Clone options to prevent shared mutation across instances
        ...
        var btnStringMap = {
            currentPage: 'currentPage', btnFirst: 'firstPage', btnPrev: 'previousPage',
            btnNext: 'nextPage', btnLast: 'lastPage', btnZoomIn: 'zoomIn',
            btnZoomOut: 'zoomOut', btnRotateLeft: 'rotateLeft', btnRotateRight: 'rotateRight',
            btnAutoplay: 'autoFlip', btnSearch: 'search', btnBookmark: 'bookmarks',
            btnNotes: 'notes', btnToc: 'tableOfContent', btnThumbs: 'pages',
            btnShare: 'share', btnPrint: 'print', btnDownloadPages: 'download',
            btnDownloadPdf: 'downloadPdf', btnSound: 'sound', btnTools: 'more',
            btnExpand: 'toggleFullscreen', btnSingle: 'toggleSinglePage', btnClose: 'close',
        };
        for (var btnKey in btnStringMap) {
            if (s[btnStringMap[btnKey]] && this.options[btnKey]) {
                this.options[btnKey].title = s[btnStringMap[btnKey]];
            }
        }
    ...
    initOptions(o) {
        ...
        if (o.deeplinkingEnabled) {
            this.checkHash();
            this._boundCheckHash = this.checkHash.bind(this);
        ...
        if (o.lightBox) {
            ...
            this._intersectionObserver.observe(this.wrapper);
        }
    }
    ...
    async destroy() {
        ...
        // Fix 2: Clean up hashchange listener
        if (this._boundCheckHash) {
            window.removeEventListener('hashchange', this._boundCheckHash);
            this._boundCheckHash = null;
        }

        // Fix 3: Clean up IntersectionObserver
        if (this._intersectionObserver) {
            this._intersectionObserver.disconnect();
            this._intersectionObserver = null;
        }

        // Clean up thumbnails IntersectionObserver
        if (this.thumbs && typeof this.thumbs.destroy === 'function') {
            this.thumbs.destroy();
        }

        if (this._boundNoteVisibility) {
            window.removeEventListener('r3d-update-note-visibility', this._boundNoteVisibility);
            this._boundNoteVisibility = null;
        }
        ...
        if (this._boundDocClickClose) {
            document.removeEventListener('click', this._boundDocClickClose);
            this._boundDocClickClose = null;
        }

        if (this.tooltip && typeof this.tooltip.destroy === 'function') {
            this.tooltip.destroy();
        }
        if (this.tooltip2 && typeof this.tooltip2.destroy === 'function') {
    ...
    initNotes() {
        ...
        const self = this;
        this._boundNoteVisibility = function (e) {
        ...
        window.addEventListener('r3d-update-note-visibility', this._boundNoteVisibility);
    }
    ...
    spotlight(url, title, description) {
        ...
        if (title || description) {
            ...
            if (title) {
                ...
                titleEl.className = 'flipbook-spotlight-title';
                titleEl.textContent = title;
                captionContainer.appendChild(titleEl);
            }
            if (description) {
                var descEl = document.createElement('div');
                descEl.className = 'flipbook-spotlight-description';
                descEl.textContent = description;
                captionContainer.appendChild(descEl);
            }
        }
    ...
    initSound() {
        if (this.options.flipSound) {
            ...
            this.flipSound.addEventListener('canplaythrough', () => {
            ...
            if (btn.enabled) {
                ...
                if (btn.name === 'currentPage') {
                    ...
                    this.$search.innerHTML =
                        '<div>' +
                        '<input class="toolbarField skin-color skin-color-bg" title="' + FLIPBOOK.escapeHtml(o.strings.findInDocument || 'Find') + '" autocapitalize="none" placeholder="' +
                        FLIPBOOK.escapeHtml(o.strings.findInDocument) +
                        '...">' +
                        '</div>' +
                    ...
                    searchInput.addEventListener('input', function () {
                        ...
                        searchTimer = setTimeout(() => {
                            self.thumbs.$findInput.value = value;
                            self.thumbs.$findInput.dispatchEvent(new Event('keyup'));
                            // or better (more realistic):
                            // self.thumbs.$findInput.dispatchEvent(new KeyboardEvent('keyup', { key: 'a', bubbles: true }));
                        }, DEBOUNCE_MS);
                    });
    ...
    playFlipSound() {
        if (this.options.sound && this.Book.enabled && this.flipSound && typeof this.flipSound.play !== 'undefined') {
    ...
    toggleToolsMenu() {
        ...
        if (!this.toolsMenu.parentNode) {
            ...
            if (!this._boundDocClickClose) {
                this._boundDocClickClose = function () {
                ...
                document.addEventListener('click', this._boundDocClickClose);
            }
        }
    ...
    togglePrintMenu() {
        ...
        if (!this.printMenu) {
            ...
            currentPageButton.innerHTML =
                '<div class="c-p skin-color flipbook-btn">' + FLIPBOOK.escapeHtml(this.strings.printCurrentPage) + '</div>';
            ...
            leftPageButton.innerHTML =
                '<div class="c-l-p skin-color flipbook-btn">' + FLIPBOOK.escapeHtml(this.strings.printLeftPage) + '</div>';
            ...
            rightPageButton.innerHTML =
                '<div class="c-r-p skin-color flipbook-btn">' + FLIPBOOK.escapeHtml(this.strings.printRightPage) + '</div>';
            ...
            allPagesButton.innerHTML = '<div class="skin-color flipbook-btn">' + FLIPBOOK.escapeHtml(this.strings.printAllPages) + '</div>';
    ...
    toggleDownloadMenu() {
        ...
        if (!this.dlMenu) {
            ...
            var currentPageButton = document.createElement('a');
            currentPageButton.innerHTML =
                '<div class="c-p skin-color flipbook-btn">' + FLIPBOOK.escapeHtml(this.strings.downloadCurrentPage) + '</div>';
            content.appendChild(currentPageButton);
            currentPageButton.addEventListener('click', function () {
            ...
            var leftPageButton = document.createElement('a');
            leftPageButton.innerHTML =
                '<div class="c-l-p skin-color flipbook-btn">' + FLIPBOOK.escapeHtml(this.strings.downloadLeftPage) + '</div>';
            content.appendChild(leftPageButton);
            leftPageButton.addEventListener('click', function () {
                self.downloadPage(self.cPage[0], this);
                self.toggleDownloadMenu();
            ...
            var rightPageButton = document.createElement('a');
            rightPageButton.innerHTML =
                '<div class="c-r-p skin-color flipbook-btn">' + FLIPBOOK.escapeHtml(this.strings.downloadRightPage) + '</div>';
            content.appendChild(rightPageButton);
            rightPageButton.addEventListener('click', function () {
            ...
            allPagesButton.innerHTML =
                '<div class="skin-color flipbook-btn">' + FLIPBOOK.escapeHtml(this.strings.downloadAllPages) + '</div>';
            content.appendChild(allPagesButton);
            allPagesButton.addEventListener('click', function () {
                var link = document.createElement('a');
    ...
    toggleNotesMenu() {
        ...
        if (!this.notesMenu) {
            ...
            this.options.noteTypes.forEach(function (type) {
                ...
                row.innerHTML =
                    '<span>' +
                    FLIPBOOK.escapeHtml(type.title) +
                    '</span>' +
                    ...
                    FLIPBOOK.escapeHtml(type.color) +
                    '"></span>';
...
FLIPBOOK.Notes = class {
    constructor(main) {
        ...
        this.notePopup.innerHTML =
            ...
            FLIPBOOK.escapeHtml(main.options.strings.typeInYourNote) +
...
FLIPBOOK.ProgressBar = class {
    ...
    _render() {
      ...
      <div class="progress-thumb" tabindex="0" role="slider" aria-valuenow="0" aria-valuemin="${parseInt(this.min, 10)}" aria-valuemax="${parseInt(this.max, 10)}"></div>
    `;
    ...
    _bindEvents() {
        ...
        this.thumb.addEventListener('mousedown', this._boundStartDrag);
        this.el.addEventListener('mousedown', this._boundStartDrag);
        window.addEventListener('mousemove', this._boundOnDrag);
        window.addEventListener('mouseup', this._boundEndDrag);

        this.thumb.addEventListener('touchstart', this._boundStartDrag, { passive: false });
        this.el.addEventListener('touchstart', this._boundStartDrag, { passive: false });
        window.addEventListener('touchmove', this._boundOnDrag, { passive: false });
    ...
    destroy() {
        ...
        window.removeEventListener('touchmove', this._boundOnDrag);
...
FLIPBOOK.Thumbnails = class {
    constructor(main) {
        ...
        currentBookmark.innerHTML =
            '<div class="c-p skin-color flipbook-btn">' + FLIPBOOK.escapeHtml(options.strings.bookmarkCurrentPage) + '</div>';
        ...
        leftBookmark.innerHTML =
            '<div class="c-l-p skin-color flipbook-btn">' + FLIPBOOK.escapeHtml(options.strings.bookmarkLeftPage) + '</div>';
        ...
        rightBookmark.innerHTML =
            '<div class="c-r-p skin-color flipbook-btn">' + FLIPBOOK.escapeHtml(options.strings.bookmarkRightPage) + '</div>';
        ...
        this.findInput.title = options.strings.findInDocument || 'Find';
        ...
        this.thumbsScroller.setAttribute('aria-live', 'polite');
        this.thumbsScroller.setAttribute('role', 'region');
        this.thumbsScroller.setAttribute('aria-label', 'Search results');
        ...
        for (let i = 0; i < options.pages.length; i++) {
            ...
            th.el.instance = th;
            this.thumbObserver.observe(th.el);
        }
    ...
    showSearchResults(matches, pageIndex, str, firstResult) {
        ...
        var esc = FLIPBOOK.escapeHtml;
        searchMatch.innerHTML = `
        <div class="flipbook-search-match-title">
            <span style="float:left"><strong>${esc(strings.page)} ${esc(displayPageTitle)}</strong></span>
            <span style="float:right">${parseInt(matches, 10)} ${esc(matchString)}</span>
        </div>
        <div class="flipbook-search-match-text">${esc(str)}</div>
    `;
...
FLIPBOOK.Lightbox = class {
    ...
    _trapFocus() {
        ...
        const overlay = FLIPBOOK.Lightbox.sharedOverlay;
        ...
        document.addEventListener('keydown', this._boundFocusTrap);
    ...
    _releaseFocusTrap() {
        if (this._boundFocusTrap) {
            document.removeEventListener('keydown', this._boundFocusTrap);
